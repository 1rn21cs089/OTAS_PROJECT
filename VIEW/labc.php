...
if (isset($_SESSION['username'])) {
    ...
    if ($subresult->num_rows > 0) {
        ...
        WHERE Lab = 1 AND c.Section = '$section' AND c.Sem = $sem AND s.USN LIKE '$usn' 
        ORDER BY c.SubCode";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $subnameResult = $conn->query("SELECT su.SubName FROM teacher_subcombo c JOIN subjects su ON su.SubCode = c.SubCode JOIN teacher t ON t.Tid = c.Tid JOIN student s ON s.Section = c.Section AND s.Sem = c.Sem WHERE c.Lab = 1 AND c.Section = '$section' AND c.Sem = $sem AND s.USN LIKE '$usn'");
while ($subnameRow = $subnameResult->fetch_assoc()) {
    echo "<option value='" . $subnameRow["SubName"] . "'" . ($subnameRow["SubName"] == $row["SubName"] ? " selected" : "") . ">" . $subnameRow["SubName"] . "</option>";
                ...
                $teachernameResult = $conn->query("SELECT CONCAT(t.Teacher_FName, ' ', t.Teacher_LName) AS TeacherName FROM teacher_subcombo c JOIN subjects su ON su.SubCode = c.SubCode JOIN teacher t ON t.Tid = c.Tid JOIN student s ON s.Section = c.Section AND s.Sem = c.Sem WHERE c.Lab = 1 AND c.Section = '$section' AND c.Sem = $sem AND s.USN LIKE '$usn'");
...
<script>
    ...
    function submitEdits() {
        ...
        form.setAttribute("action", "savelab.php");
